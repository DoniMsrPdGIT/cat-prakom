		...
		<div class="row">
			...
			<div class="col-sm-4">
				<div class="pull-right">
					<a href="<?=base_url('soal_twk/add')?>" class="btn bg-purple btn-flat btn-sm"><i class="fa fa-plus"></i> Buat Soal</a>
					<button type="button" onclick="reload_ajax()" class="btn btn-flat btn-sm bg-maroon"><i class="fa fa-refresh"></i> Reload</button>
				</div>
			</div>
		</div>
    </div>
	<?=form_open('soal_twk/delete', array('id'=>'bulk'))?>
...
<script src="<?=base_url()?>assets/dist/js/app/soal_twk/data.js"></script>
...
$(document).ready(function(){
	$('#matkul_filter').on('change', function(){
		...
		let src = '<?=base_url()?>soal_twk/data';
		let url;
