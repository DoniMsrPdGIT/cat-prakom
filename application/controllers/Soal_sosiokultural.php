...
class Soal_sosiokultural extends CI_Controller {
    ...
    public function index()
	{
        ...
        if($this->ion_auth->is_admin()){
            ...
            $data['matkul'] = $this->soal->getMatkulDosenDasar($user->username,$jenis);
        }

		$this->load->view('_templates/dashboard/_header.php', $data);
		$this->load->view('soal_sosiokultural/data');
...
$ujian_id = $id;
		...
		$data = [
			...
			'judul'	    => 'Soal',
            'subjudul'  => 'Edit Soal',
            'soal'      => $this->soal->getSoalByIdSoalSosiokultural($id),
        ];

        $this->load->view('_templates/dashboard/_header.php', $data);
		$this->load->view('soal_sosiokultural/detail');
		$this->load->view('_templates/dashboard/_footer.php');
    }
    ...
    public function add()
	{
        ...
        if($this->ion_auth->is_admin()){
            ...
            //Jika bukan maka matkul dipilih otomatis sesuai matkul dosen
            $data['dosen'] = $this->soal->getMatkulDosenDasar($user->username,$jenis);
        }

		$this->load->view('_templates/dashboard/_header.php', $data);
		$this->load->view('soal_sosiokultural/add');
		$this->load->view('_templates/dashboard/_footer.php');
    }
...
$ujian_id = $id;
		...
		$this->load->view('_templates/dashboard/_header.php', $data);
		$this->load->view('soal_sosiokultural/edit');
		$this->load->view('_templates/dashboard/_footer.php');
	}

	public function data($id=null, $dosen=null)
	{
		$this->output_json($this->soal->getDataSoalSosiokultural($id, $dosen), false);
    }
    ...
    public function save()
    {
        ...
        if($this->form_validation->run() === FALSE){
            ...
            $bobot='A:'.$this->input->post('option_a').',B:'.$this->input->post('option_b').',C:'.$this->input->post('option_c').',D:'.$this->input->post('option_d').',E:'.$this->input->post('option_e');
            $data = [
            ...
            foreach ($_FILES as $key => $val) {
                ...
                $getsoal = $this->soal->getSoalByIdSoalSosiokultural($this->input->post('id_soal', true));
            ...
            if($method==='add'){
                ...
                //insert data
                $this->master->create('tb_soal_sosiokultural', $data);
            }else if($method==='edit'){
                //push array
                $data['updated_on'] = time();
                //update data
                $id_soal = $this->input->post('id_soal', true);
                $this->master->update('tb_soal_sosiokultural', $data, 'id_soal', $id_soal);
            }else{
                show_error('Method tidak diketahui', 404);
            }
            redirect('soal_sosiokultural');
        }
    }
    ...
    public function delete()
    {
        ...
        if(!$chk){
            ...
            if($this->master->delete('tb_soal_sosiokultural', $chk, 'id_soal')){
